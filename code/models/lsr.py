...
class LSR(nn.Module):
    def __init__(self, config):
        ...
        self.word_emb = nn.Embedding(
            config.data_word_vec.shape[0], config.data_word_vec.shape[1]
        )
        ...
        input_size = (
            config.data_word_vec.shape[1] + config.coref_size + config.entity_type_size
        )  # + char_hidden
        ...
        self.bili = torch.nn.Bilinear(
            hidden_size + config.dis_size, hidden_size + config.dis_size, hidden_size
        )
        ...
        self.rnn_sent = Encoder(
            input_size, hidden_size, config.dropout_emb, config.dropout_rate
        )
    ...
    def doc_encoder(self, input_sent, context_seg):
        ...
        for batch_no in range(batch_size):
            ...
            sent_lens_t = torch.Tensor(sent_lens).to(input_sent).long()
    ...
    def forward(
        ...
        output = torch.bmm(
            mention_node_position[:, :max_doc_len, :max_mention_num],
