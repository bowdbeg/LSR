...
class Config(object):
    def __init__(self, args):
        ...
        self.h_t_limit = 105000

        self.test_batch_size = self.batch_size
        self.test_relation_limit = 105000
        self.char_limit = 16
        self.sent_limit = 25
    ...
    def load_train_data(self):
        ...
        self.data_train_word = np.load(
            os.path.join(self.data_path, prefix + "_word.npy")
        )

        # elmo_ids = batch_to_ids(batch_words).to(self.device)
        self.data_train_pos = np.load(os.path.join(self.data_path, prefix + "_pos.npy"))
        self.data_train_ner = np.load(
            os.path.join(self.data_path, prefix + "_ner.npy")
        )  # word_embedding
        self.data_train_char = np.load(
            os.path.join(self.data_path, prefix + "_char.npy")
        )
        self.data_train_seg = np.load(os.path.join(self.data_path, prefix + "_seg.npy"))
        self.data_train_node_position = np.load(
            os.path.join(self.data_path, prefix + "_node_position.npy")
        )

        self.data_train_node_position_sent = np.load(
            os.path.join(self.data_path, prefix + "_node_position_sent.npy")
        )

        self.data_train_node_sent_num = np.load(
            os.path.join(self.data_path, prefix + "_node_sent_num.npy")
        )

        self.data_train_node_num = np.load(
            os.path.join(self.data_path, prefix + "_node_num.npy")
        )
        self.data_train_entity_position = np.load(
            os.path.join(self.data_path, prefix + "_entity_position.npy")
        )
        self.train_file = json.load(
            open(os.path.join(self.data_path, prefix + ".json"))
        )

        self.data_train_sdp_position = np.load(
            os.path.join(self.data_path, prefix + "_sdp_position.npy")
        )
        self.data_train_sdp_num = np.load(
            os.path.join(self.data_path, prefix + "_sdp_num.npy")
        )
    ...
    def load_test_data(self):
        ...
        self.data_test_char = np.load(
            os.path.join(self.data_path, prefix + "_char.npy")
        )

        self.data_test_node_position = np.load(
            os.path.join(self.data_path, prefix + "_node_position.npy")
        )

        self.data_test_node_position_sent = np.load(
            os.path.join(self.data_path, prefix + "_node_position_sent.npy")
        )
        # self.data_test_adj = np.load(os.path.join(self.data_path, prefix+'_adj.npy'))

        self.data_test_node_sent_num = np.load(
            os.path.join(self.data_path, prefix + "_node_sent_num.npy")
        )

        self.data_test_node_num = np.load(
            os.path.join(self.data_path, prefix + "_node_num.npy")
        )
        self.data_test_entity_position = np.load(
            os.path.join(self.data_path, prefix + "_entity_position.npy")
        )
        ...
        self.data_test_sdp_position = np.load(
            os.path.join(self.data_path, prefix + "_sdp_position.npy")
        )
        self.data_test_sdp_num = np.load(
            os.path.join(self.data_path, prefix + "_sdp_num.npy")
        )
        ...
        self.test_order.sort(
            key=lambda x: np.sum(self.data_test_word[x] > 0), reverse=True
        )

    def get_train_batch(self):

        random.shuffle(self.train_order)
        context_idxs = torch.LongTensor(self.batch_size, self.max_length).to(
            self.device
        )
        context_pos = torch.LongTensor(self.batch_size, self.max_length).to(self.device)
        h_mapping = torch.Tensor(self.batch_size, self.h_t_limit, self.max_length).to(
            self.device
        )
        t_mapping = torch.Tensor(self.batch_size, self.h_t_limit, self.max_length).to(
            self.device
        )
        ...
        relation_label = torch.LongTensor(self.batch_size, self.h_t_limit).to(
            self.device
        )
        ...
        node_position = (
            torch.zeros(self.batch_size, self.max_node_num, self.max_length)
            .float()
            .to(self.device)
        )

        sdp_position = (
            torch.zeros(self.batch_size, self.max_entity_num, self.max_length)
            .float()
            .to(self.device)
        )
        ...
        node_sent_num = (
            torch.zeros(self.batch_size, self.max_sent_num).float().to(self.device)
        )
        ...
        for b in range(self.train_batches):
            ...
            cur_batch.sort(
                key=lambda x: np.sum(self.data_train_word[x] > 0), reverse=True
            )
            ...
            for i, index in enumerate(cur_batch):
                ...
                context_pos[i].copy_(
                    torch.from_numpy(self.data_train_pos[index, :])
                )  # ???
                context_char_idxs[i].copy_(
                    torch.from_numpy(self.data_train_char[index, :])
                )
                ...
                node_position[i].copy_(
                    torch.from_numpy(self.data_train_node_position[index])
                )
                ...
                node_sent_num[i].copy_(
                    torch.from_numpy(self.data_train_node_sent_num[index])
                )

                node_num[i].copy_(torch.from_numpy(self.data_train_node_num[index]))
                entity_position[i].copy_(
                    torch.from_numpy(self.data_train_entity_position[index])
                )
                ...
                sdp_position[i].copy_(
                    torch.from_numpy(self.data_train_sdp_position[index])
                )
            ...
            entity_mention_num = list(
                map(add, entity_num, node_num[:cur_bsz].squeeze(1).tolist())
            )
            ...
            yield {
                ...
                "sdp_position": sdp_position[
                    :cur_bsz, :max_sdp_num, :max_c_len
                ].contiguous(),
                "sdp_num": sdp_nums,
            }

    def get_test_batch(self):
        context_idxs = torch.LongTensor(self.test_batch_size, self.max_length).to(
            self.device
        )
        context_pos = torch.LongTensor(self.test_batch_size, self.max_length).to(
            self.device
        )
        ...
        context_ner = torch.LongTensor(self.test_batch_size, self.max_length).to(
            self.device
        )
        context_char_idxs = torch.LongTensor(
            self.test_batch_size, self.max_length, self.char_limit
        ).to(self.device)
        relation_mask = torch.Tensor(self.test_batch_size, self.h_t_limit).to(
            self.device
        )
        ht_pair_pos = torch.LongTensor(self.test_batch_size, self.h_t_limit).to(
            self.device
        )
        context_seg = torch.LongTensor(self.batch_size, self.max_length).to(self.device)
        ...
        for b in range(self.test_batches):
            ...
            for i, index in enumerate(cur_batch):
                ...
                context_char_idxs[i].copy_(
                    torch.from_numpy(self.data_test_char[index, :])
                )
                ...
                node_position[i].copy_(
                    torch.from_numpy(self.data_test_node_position[index])
                )
                node_position_sent[i].copy_(
                ...
                node_sent_num[i].copy_(
                    torch.from_numpy(self.data_test_node_sent_num[index])
                )

                node_num[i].copy_(torch.from_numpy(self.data_test_node_num[index]))
                entity_position[i].copy_(
                    torch.from_numpy(self.data_test_entity_position[index])
                )
                entity_num.append(len(ins["vertexSet"]))
                sentence_num.append(len(ins["sents"]))
                sentence_len.append(
                ...
                sdp_position[i].copy_(
                    torch.from_numpy(self.data_test_sdp_position[index])
                )
    ...
    def train(self, model_pattern, model_name):
        ...
        for epoch in range(self.max_epoch):
            ...
            for no, data in enumerate(tqdm(self.get_train_batch())):
                ...
                loss = torch.sum(
                    BCE(predict_re, relation_multi_label) * relation_mask.unsqueeze(2)
